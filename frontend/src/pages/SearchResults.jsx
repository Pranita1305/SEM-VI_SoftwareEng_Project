...
/* ─── Data ─────────────────────────────────────── */
const RIDE_OPTIONS = [
  { type: "UberGo",     icon: "🚖", baseRate: 12, capacity: "4 seats", description: "Affordable everyday rides",  color: "#4f9cf9", tag: "Most Popular" },
  { type: "Uber Premier", icon: "🚘", baseRate: 18, capacity: "4 seats", description: "Comfortable sedans",         color: "#7c5cfc", tag: null },
  { type: "Uber XL",    icon: "🚐", baseRate: 22, capacity: "6 seats", description: "For larger groups",          color: "#fbbf24", tag: null },
  { type: "Uber Auto",  icon: "🛺", baseRate: 8,  capacity: "3 seats", description: "Budget-friendly autos",      color: "#34d399", tag: "Cheapest" },
];
...
function estimateDistance(src, dst) {
  const seed = (src.length * 7 + dst.length * 13) % 25;
...
function RideCard({ opt, distance, surge, eta, isSelected, onClick }) {
  const fare = Math.round(opt.baseRate * distance * surge);
    ...
    >
      ...
      <div style={{ padding: "1rem 1.25rem 1.25rem", display: "flex", alignItems: "center", gap: "1rem", minWidth: 0 }}>
        ...
        <div style={{ textAlign: "right", flexShrink: 0 }}>
          <div style={{ fontWeight: 800, fontSize: "1.3rem", color: opt.color, letterSpacing: "-0.02em" }}>₹{fare}</div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.85rem" }}>
            ...
            <div>
              ...
              <div style={{ fontSize: "0.78rem", color: "var(--text-2)" }}>
                ₹{Math.round(selectedOpt.baseRate * distance * surge)} &nbsp;·&nbsp; ~{eta} min &nbsp;·&nbsp; {selected?.capacity}
              </div>
            </div>
          </div>
          <button
